...
def define_grey(opt):
    ...
    net_grey = InvComNet(rgb_type, subnet(subnet_type, init), opt_net['block_num'], use_robust, Gau_channel_scale)

    return net_grey

## 用于将 整数空间数值转换到浮点数空间，增加输入数据的鲁棒性
def define_robust(opt):
    opt_net = opt['network_robust']

    if opt_net['cin']:
        channel_in = opt_net['cin']
    if opt_net['mid_ch']:
        mid_ch = opt_net['mid_ch']

    net_robust = Robust_Module(channel_in=channel_in, mid_ch=mid_ch)
