...
class IRNColorModel(BaseModel):
    def __init__(self, opt):
        ...
        self.Distribution = "Gaussian" ## Gaussian   laplacian
        print("Distribution option ==== ", self.Distribution)
        ...
        if 0:
            ...
            self.netG.to('cuda')
            flops, params = profile(self.netG, inputs=(input_data,))
    ...
    def laplace_batch(self, dims, scale=1.0):
        # 标准拉普拉斯分布 Laplace(0, scale)
        u = torch.rand(tuple(dims)).to(self.device) - 0.5  # 均匀分布 [-0.5, 0.5]
        eps = 1e-10
        return -scale * torch.sign(u) * torch.log(1 - 2 * torch.abs(u) + eps)
    ...
    def loss_forward(self, out, y, z):
        ...
        # print("laplacian")
        DIST_MAP = {
            'Gaussian': lambda z: torch.sum(z ** 2),
            'laplacian': lambda z: torch.sum(torch.abs(z)),
            'uniform': lambda z: torch.sum((torch.abs(z) - 3.0).clamp(min=0) ** 2)
        }

        l_forw_ce = self.train_opt['lambda_ce_forw'] * DIST_MAP[dist_type](z) / z.shape[0]

        return l_forw_fit, l_forw_ce
    def loss_backward(self, x, y, x_LQ):
        if self.use_robust:
            x_samples, x_samples_EN = self.netG(x=y, rev=True)
    ...
    def optimize_parameters(self, step):
        ...
        if "Gaussian" == self.Distribution:
            y_ = torch.cat((out_LQ, gaussian_scale * self.gaussian_batch(zshape)), dim=1)
        elif "laplacian" == self.Distribution:
            y_ = torch.cat((out_LQ, gaussian_scale * self.laplace_batch(zshape, scale=1.0)), dim=1)
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            if "Gaussian" == self.Distribution:
                ...
                y_forw = torch.cat((self.forw_L, gaussian_scale * self.laplace_batch(zshape)), dim=1)
            ...
            time_end = time.time()
        self.time_diff = time_end - time_start
        self.netG.train()
    ...
    def get_current_visuals(self):
        ...
        out_dict['en_time'] = self.time_diff
        # if self.use_robust:
