...
class Robust_Module(nn.Module):
    def __init__(self, channel_in=3, mid_ch=64):
        ...
        # self.Conv_atten1 = nn.Sequential(nn.Conv2d(channel_in, mid_ch, 1),
        #               ResidualUnit(mid_ch),
        #               ResidualUnit(mid_ch),
        #               ResidualUnit(mid_ch),
        #               AttentionBlock(mid_ch),
        #               nn.Conv2d(mid_ch, channel_in, 1))
        ...
        # self.Conv_atten4 = nn.Sequential(nn.Conv2d(mid_ch, mid_ch, 1),
        #                                  ResidualUnit(mid_ch),
        #                                  ResidualUnit(mid_ch),
        #                                  ResidualUnit(mid_ch),
        #                                  AttentionBlock(mid_ch),
    ...
    def forward(self, x):
        # x = torch.cat((x_LQ, X_Inv), dim=1)
        # print("xLQ shape", x_LQ.shape, X_Inv.shape)
...
class InvComNet(nn.Module):
    ...
    def forward(self, x, x_LQ=None, rev=False, cal_jacobian=False):
        out = x
        jacobian = 0
        if not rev:
