...
## refer to "Attention-Based Multimodal Image Feature
# Fusion Module for Transmission Line Detection"
...
class f_module(nn.Module):
    def __init__(self, c, reduction_ratio=2):
        ...
        cat_dim = dim1+dim1+dim1+dim1+dim1+dim1

        self.fc_vl_atten = nn.Sequential(
            nn.Conv2d(cat_dim, c, kernel_size=1),
            nn.BatchNorm2d(c)
        )
        ...
        # if self.reduce != 0:
            ## refer to CBAM
        self.Channel_gate = ChannelGate(cat_dim, reduction_ratio=reduction_ratio)
    ...
    def forward(self, inputx, inputir):
        ...
        cat_feat = self.Channel_gate(cat_feat)
...
class Unet(nn.Module):
    def __init__(self, In_channel=3, bilinear=False):
        super(Unet, self).__init__()
        self.bilinear = bilinear

        self.inc = DoubleConv(In_channel, 32)
        self.inc_ir = DoubleConv(In_channel, 32)
        ...
        self.sf1 = f_module(32, reduction_ratio=2)
        self.sf2 = f_module(64, reduction_ratio=2)
        self.sf3 = f_module(128, reduction_ratio=2)
        self.sf4 = f_module(256, reduction_ratio=1)
        self.sf5 = f_module(512, reduction_ratio=1)
        ...
        self.outc = OutConv(32, In_channel)
